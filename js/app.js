...
(function () {
  ...
  // MIDI support removed: no MidiManager
  var drums = null;
  ...
  $('power-btn').addEventListener('click', async function () {
    ...
    try {
      ...
      status('Audio init failed: ' + e.message);
    ...
    return function (ev) {
      if (document.body.classList.contains('learn-mode')) {
        ...
        if (actionId) {
          ...
          // MIDI binding disabled in this build; show status instead
          status('Armed mapping: ' + actionId);
        }
        ev.preventDefault();
        ev.stopPropagation();
        return;
