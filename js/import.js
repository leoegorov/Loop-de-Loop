/* Import: load previously exported zips (WAV stems + manifest.json) or loose
   .wav files back into fresh loop channels. Zip entries may be stored (our own
...
(function () {
    ...
    try {
      ...
      while (p + 8 <= bytes.length) {
        ...
        if (id === 'acid' && size >= 24) {
          ...
          if (bpm > 30 && bpm < 300) {
            return { bpm: bpm, beats: beats > 0 ? beats : null };
          }
          return null;
        }
        p += 8 + size + (size & 1);
  ...
  async function importFiles(engine, files, addChannelFn, status) {
    ...
    if (!order.length) throw new Error('no .wav files found');
    ...
    for (var o = 0; o < order.length; o++) {
      ...
      if (tr.wav) {
        ...
        if (!manifest) {
          ...
          if (acid) srcInfo = { bpm: acid.bpm, beats: acid.beats, source: 'wav tempo data' };
          if (!srcInfo) {
